...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _smooth_action_sequence(actions: list, window_radius: int = 5) -> list:
        if len(actions) <= window_radius * 2:
            return actions
        ...
        original_dtype = action_array.dtype
        action_array = action_array.astype(np.float64, copy=False)
        smoothed_actions = action_array.copy()
        kernel_size = window_radius * 2 + 1
        kernel = np.ones(kernel_size, dtype=np.float64) / kernel_size
        padded_actions = np.pad(action_array, ((window_radius, window_radius), (0, 0)), mode="edge")

        for action_index in range(action_array.shape[1]):
            smoothed_actions[:, action_index] = np.convolve(
                padded_actions[:, action_index], kernel, mode="valid"
            )

        smoothed_actions[:window_radius] = action_array[:window_radius]
        smoothed_actions[-window_radius:] = action_array[-window_radius:]
        return [row.astype(original_dtype, copy=False) for row in smoothed_actions]
